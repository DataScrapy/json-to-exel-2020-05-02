        ...
        try:
            ...
            for i, product in enumerate(product_all_lst):
                try:
                    ...
                    product_description = str(product["body_html"])

                    if "<" in product_description or ">" in product_description:
                        while True:
                            if product_description.startswith("<"):
                                str_position = product_description.find(">")
                                product_description = product_description[str_position + 1:].strip()
                            else:
                                break

                        if "<" in product_description:
                            str_position = product_description.find("<")
                            product_description = product_description[:str_position].strip()
                    ...
                    for j, variant in enumerate(variants_lst):
                        try:
                            ...
                            if (
                                    ...
                                    item_name.lower().__contains__("Women's T-Shirt".lower()) or
                                    item_name.lower().__contains__("Men's T-Shirt, Regular".lower()) or
                                    item_name.lower().__contains__("Men's T-Shirt".lower())
                            ):
